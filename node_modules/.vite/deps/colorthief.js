...
async function loadPixels(source, options) {
  checkAborted(options == null ? void 0 : options.signal);
  const loader2 = await getLoader(options == null ? void 0 : options.loader);
  return loader2.load(source, options == null ? void 0 : options.signal);
}
...
async function getPalette(source, options) {
  ...
  checkAborted(options == null ? void 0 : options.signal);
  if (options == null ? void 0 : options.worker) {
    ...
    if (isWorkerSupported2()) {
      ...
      return extractInWorker2(pixelArray, opts.colorCount, options == null ? void 0 : options.signal);
    }
  }
  const [pixels, quantizer] = await Promise.all([
    loadPixels(source, options),
    getQuantizer(options == null ? void 0 : options.quantizer)
  ]);
  checkAborted(options == null ? void 0 : options.signal);
...
function getPaletteSync(source, options) {
  const opts = validateOptions(options ?? {});
  const quantizer = (options == null ? void 0 : options.quantizer) ?? defaultQuantizer;
